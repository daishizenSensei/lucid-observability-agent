...
export function buildDiagnosis(config: AgentConfig, input: DiagnosisInput) {
  ...
  for (const pattern of config.diagnosisPatterns) {
    if (
      pattern.keywords.some(
        (kw) =>
          title.includes(kw) || culprit.includes(kw) || stack.includes(kw),
      )
    ) {
  ...
  if (category === 'application_error') {
    if (
      ...
      title.includes('enotfound') ||
      title.includes('econnreset') ||
      ...
      suggestions.push(
        {
          ...
          description: 'Check upstream provider health',
          confidence: 'high',
        },
        {
          action: 'add_retry',
      ...
      suggestions.push(
        {
          ...
          description: 'Switch to streaming for long operations',
          confidence: 'high',
        },
        {
          action: 'check_pool',
        ...
        },
        {
          ...
          description: 'Review timeout values',
          confidence: 'medium',
        },
      );
      relatedPatterns.push(
      ...
      suggestions.push(
        {
          action: 'rotate_keys',
          description: 'Check if API keys have expired',
          confidence: 'high',
        },
        {
          action: 'check_config',
      ...
      suggestions.push(
        {
          action: 'check_quotas',
          description: 'Review quota/rate limit settings',
          confidence: 'high',
        },
        {
          action: 'add_queuing',
          description: 'Add request queuing',
          confidence: 'medium',
        },
      );
      relatedPatterns.push(
      ...
      title.includes('unique constraint') ||
      ...
      suggestions.push(
        ...
        {
          action: 'check_pool',
          description: 'Review connection pool config',
          confidence: 'medium',
        },
        {
          action: 'check_migrations',
          description: 'Ensure migrations are applied',
          confidence: 'medium',
        },
      );
      relatedPatterns.push(
      ...
      suggestions.push(
        ...
        {
          action: 'add_error_detail',
          description: 'Return validation errors in API response',
          confidence: 'high',
        },
      );
      relatedPatterns.push(
      ...
      suggestions.push(
        ...
        {
          action: 'check_streams',
          description: 'Verify streams are closed',
          confidence: 'medium',
        },
      );
      relatedPatterns.push(
      ...
      suggestions.push(
        {
          action: 'investigate',
          description: 'Examine the full stack trace',
          confidence: 'high',
        },
        {
          action: 'add_context',
          description: 'Add breadcrumbs around the error',
          confidence: 'medium',
        },
      );
    }
  ...
  for (const bug of config.knownBugs) {
    if (
      bug.keywords.some(
        (kw) => title.includes(kw) || stack.includes(kw),
      )
    ) {
      suggestions.unshift({
        action: 'known_bug',
        description: `Known bug: ${bug.title} — ${bug.fix}`,
        confidence: 'high',
      });
    }
  }
  ...
  const recentActivity =
    Date.now() - new Date(input.lastSeen).getTime() < 1000 * 60 * 60;
  let severity: 'critical' | 'high' | 'medium' | 'low';
  if (
    input.level === 'fatal' ||
    input.count > 1000 ||
    (input.count > 100 && recentActivity)
  )
    severity = 'critical';
  else if (
    input.level === 'error' &&
    (input.count > 100 || input.userCount > 10)
  )
    severity = 'high';
  ...
  const ageDays =
    (Date.now() - new Date(input.firstSeen).getTime()) /
    (1000 * 60 * 60 * 24);
  ...
  return {
    ...
    context: {
      service,
      repo: serviceInfo?.repo || 'unknown',
      runtime: serviceInfo?.runtime || 'unknown',
      environment: envTag || 'unknown',
      eventCount: input.count,
      userCount: input.userCount,
      age: `${ageDays.toFixed(1)} days`,
      lastActive: recentActivity ? 'within last hour' : input.lastSeen,
    },
    suggestions,
    relatedPatterns,
    nextSteps: [
      ...
      severity === 'critical' || severity === 'high'
        ? 'URGENT: Follow runbook mitigation steps'
        : 'Monitor frequency trend',
    ],
    breadcrumbs:
      input.breadcrumbs.length > 0 ? input.breadcrumbs : undefined,
  };
}
...
export function createDiagnoseIssueTool(deps: ToolDeps): ToolDefinition {
  const { config } = deps;
    ...
    execute: async ({ issueId }) => {
      try {
        ...
        const diagnosis = buildDiagnosis(config, {
          ...
          project: String(
            (issue.project as Record<string, unknown>)?.slug || 'unknown',
          ),
        });
...
export function createCrossCorrelateTool(deps: ToolDeps): ToolDefinition {
  const { config } = deps;
  ...
  return {
    ...
    execute: async ({ traceId, runId }) => {
      if (!traceId && !runId)
        return err('Provide at least one of traceId or runId');
      ...
      try {
        ...
        for (const project of projects) {
          if (traceId)
            queries.push({ project, query: `trace_id:${traceId}` });
        ...
        allIssues.sort(
          (a, b) =>
            new Date(b.lastSeen as string).getTime() -
            new Date(a.lastSeen as string).getTime(),
        );
        const services = [
          ...new Set(allIssues.map((i) => i.project).filter(Boolean)),
        ];
